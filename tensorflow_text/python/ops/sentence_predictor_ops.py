...
class NextSentencePredictor():
  """A class for extracting segments and is_next_sentence labels from text.

  `NextSentencePredictor` takes in batches of sentences and extracts either
  the subsequent sentence or injects a random sentence in the document. It
  also returns a label the describes whether or not a random injection
  occured.
  """
  ...
  def __init__(self,
    ...
    """Creates an instance of the `NextSentencePredictor`.
    ...
    Args:
      ...
      random_next_sentence_threshold: A float threshold that determines whether
        ...
        higher the threshold the
    """
    if shuffle_fn is None:
      shuffle_fn = random_ops.random_shuffle
    self._shuffle_fn = shuffle_fn

    if random_fn is None:
      random_fn = random_ops.random_uniform
  ...
  def get_segments(self, sentences):
    ...
    Args:
      sentences: A `RaggedTensor` of strings w/ shape [batch, ..., (sentences)].

    Returns:
      A tuple of (segment_a, segment_b, is_next_sentence) where:

      segment_a: A `RaggedTensor` of strings w/ shape [..., num_sentences] that
        contains all the original sentences.
      segment_b:  A `RaggedTensor` with shape [..., num_sentences] that contain
        either the subsequent sentence of `segment_a` or a randomly injected
        sentence.
      is_next_sentence: A `RaggedTensor` of bool w/ shape [..., num_sentences]
        that contains whether or not `segment_b` is truly a subsequent sentnece
        or not.
    """
